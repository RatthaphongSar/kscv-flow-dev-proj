...
export const useMessageReadReceipts = (roomId?: string) => {
  ...
  const fetchReadReceipts = useCallback(
    async (messageIds: string[]) => {
      ...
      try {
        ...
        const data: MessageReadReceipt[] = await api(`/chat/rooms/${roomId}/messages/read-receipts?${queryString}`)
  ...
  const getMessageReaders = useCallback(
    async (messageId: string): Promise<MessageReader[]> => {
      ...
      try {
        const data: MessageReader[] = await api(`/chat/rooms/${roomId}/messages/${messageId}/readers`)
        return data
      } catch (err) {
